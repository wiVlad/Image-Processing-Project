...
while camera.isOpened():
    ...
    if not init_once:
        ok = tracker.add(cv.TrackerTLD_create(), image, bbox1)
        # ok = tracker.add(cv.TrackerTLD_create(), image, bbox2)
        # ok = tracker.add(cv.TrackerMIL_create(), image, bbox3)
        init_once = True

    ok, boxes = tracker.update(image)
    print("OK is {}, boxes are".format(ok))
    ...
    n_image = cv.flip(image, 1)
    cv.imshow('tracking', n_image)
    k = cv.waitKey(1)
